...
class LoadVideoPath:
    ...
    # 返回 image, frame_count, audio, filename
    RETURN_TYPES = (imageOrLatent, "INT", "AUDIO", "STRING")
    RETURN_NAMES = ("IMAGE", "frame_count", "audio", "filename")

    FUNCTION = "load_video"

    def load_video(self, **kwargs):
        video_path = kwargs['video']
        if video_path is None or not validate_path(video_path):
            raise Exception("video is not a valid path: " + video_path)
        ...
        # 调用原始 load_video
        image, frame_count, audio, _ = load_video(**kwargs)  # 原来的 video_info 可以忽略

        # 提取文件名
        filename = os.path.basename(video_path)

        return image, frame_count, audio, filename
